...
import automl.loggers
from automl.internal.display import CommonDisplay
from automl.internal.logging import create_logger, get_logger, redirect_output
from automl.internal.memory import get_memory
from automl.internal.pipeline import Pipeline as InternalPipeline
from automl.internal.plots.helper import MatplotlibDefaultDPI
from automl.internal.plots.yellowbrick import show_yellowbrick_plot
from automl.internal.pycaret_experiment.pycaret_experiment import _PyCaretExperiment
from automl.internal.validation import is_sklearn_cv_generator
from automl.loggers.base_logger import BaseLogger
from automl.loggers.comet_logger import CometLogger
from automl.loggers.dagshub_logger import DagshubLogger
from automl.loggers.mlflow_logger import MlflowLogger
from automl.loggers.wandb_logger import WandbLogger
...
class _TabularExperiment(_PyCaretExperiment):
    ...
    def _get_groups(
        ...
        return automl.utils.generic.get_groups(groups, data, fold_groups)
    ...
    def _get_cv_splitter(
        ...
        return automl.utils.generic.get_cv_splitter(
            fold,
    ...
    def _get_model_id(self, e, models=None) -> str:
        ...
        return automl.utils.generic.get_model_id(e, models)
    ...
    def _convert_log_experiment(
    ...
    ) -> Union[bool, automl.loggers.DashboardLogger]:
        if not (
            (
        ...
        if log_experiment:
            ...
            if loggers_list:
                return automl.loggers.DashboardLogger(loggers_list)
        return False
    ...
    def _initialize_setup(
        ...
        self.logger.info(f"automl {type(self).__name__}")
    ...
    def _plot_model(
        ...
        with patch(
            ...
            automl.internal.patches.yellowbrick.is_estimator,
        ):
            with patch(
                ...
                def residuals_interactive():
                    from automl.internal.plots.residual_plots import (
    ...
    def deploy_model(
        ...
        return automl.internal.persistence.deploy_model(
    ...
    def save_model(
        ...
        **kwargs,
        ...
        model_, model_filename = automl.internal.persistence.save_model(
            model=model,
    ...
    def load_model(
        ...
        model = automl.internal.persistence.load_model(
...
import pandas as pd
from automl.{self._ml_usecase.name.lower()} import load_model, predict_model
