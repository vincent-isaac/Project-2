...
import automl.internal.patches.sklearn
import automl.internal.patches.yellowbrick
import automl.internal.persistence
import automl.internal.preprocess
from automl.containers.metrics.classification import (
    get_all_metric_containers as get_all_class_metric_containers,
)
from automl.containers.metrics.regression import (
    get_all_metric_containers as get_all_reg_metric_containers,
)
from automl.internal.display import CommonDisplay, DummyDisplay
from automl.internal.distributions import (
...
from automl.internal.logging import get_logger, redirect_output
from automl.internal.meta_estimators import (
    CustomProbabilityThresholdClassifier,
    get_estimator_from_meta_estimator,
)
from automl.internal.metrics import EncodedDecodedLabelsReplaceScoreFunc, get_pos_label
from automl.internal.parallel.parallel_backend import ParallelBackend
from automl.internal.patches.sklearn import fit_and_score as fs
from automl.internal.pipeline import (
    Pipeline,
    estimator_pipeline,
    get_pipeline_estimator_label,
    get_pipeline_fit_kwargs,
)
from automl.internal.pycaret_experiment.tabular_experiment import _TabularExperiment
from automl.internal.tunable import TunableMixin
from automl.internal.validation import is_fitted, is_sklearn_cv_generator
from automl.utils._dependencies import _check_soft_dependencies
from automl.utils.constants import DATAFRAME_LIKE, LABEL_COLUMN, SCORE_COLUMN
...
class _SupervisedExperiment(_TabularExperiment):
    ...
    def _calculate_metrics(
        ...
        **additional_kwargs,
        ...
        """
        from automl.utils.generic import calculate_metrics
    ...
    def _get_cv_n_folds(self, fold, X, y=None, groups=None):
        import automl.utils.generic

        return automl.utils.generic.get_cv_n_folds(
        ...
        **kwargs,
        ...
        if estimator_id is None:
            if custom_grid is None:
                raise ValueError(
                    "When passing a model not in automl's model library, the custom_grid parameter must be provided."
                )
        ...
        from automl.internal.tunable import VotingClassifier, VotingRegressor
        ...
        if custom_grid is not None:
            ...
            if not (
                ...
                raise TypeError(
                    f"For the combination of search_library {search_library} and search_algorithm {search_algorithm}, automl Distribution objects are not supported. Pass a list or other object supported by the search library (in most cases, an object with a 'rvs' function)."
                )
        ...
        with estimator_pipeline(self.pipeline, model) as pipeline_with_model:
            ...
            if search_library == "scikit-learn":
                ...
                with patch(
                    ...
                    automl.internal.patches.sklearn._mp_sample_without_replacement,
                ):
                    with patch(
                        "sklearn.model_selection._search.ParameterGrid.__getitem__",
                        automl.internal.patches.sklearn._mp_ParameterGrid_getitem,
                    ):
    ...
    def add_metric(
        ...
        **kwargs:
        ...
        if self._ml_usecase == MLUsecase.CLASSIFICATION:
            new_metric = (
                automl.containers.metrics.classification.ClassificationMetricContainer(
                    id=id,
                    name=name,
            ...
            new_metric = (
                automl.containers.metrics.time_series.TimeSeriesMetricContainer(
                    id=id,
                    name=name,
            ...
            new_metric = (
                automl.containers.metrics.regression.RegressionMetricContainer(
                    id=id,
                    name=name,
                    score_func=score_func,
                    args=kwargs,
