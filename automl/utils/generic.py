...
import automl.containers
from automl.internal.logging import get_logger
from automl.internal.validation import (
...
if TYPE_CHECKING:
    from automl.internal.pycaret_experiment.pycaret_experiment import (
        _PyCaretExperiment,
    )
...
def get_config(variable: str, globals_d: dict):
    ...
    if variable not in globals_d["automl_globals"]:
        raise ValueError(
            f"Variable {variable} not found. Possible variables are: {globals_d['automl_globals']}"
        )
...
def set_config(variable: str, value, globals_d: dict):
    ...
    if variable not in globals_d["automlt_globals"] or variable == "automl_globals":
        raise ValueError(
            f"Variable {variable} not found. Possible variables are: {globals_d['automl_globals']}"
        )
...
def save_config(file_name: str, globals_d: dict):
    ...
    globals_to_dump = {
        ...
        if k in globals_d["automl_globals"] and k not in globals_to_ignore
    }
...
def get_model_id(
    e, all_models: Dict[str, "automl.containers.models.ModelContainer"]
) -> str:
...
def get_model_name(
    e,
    all_models: Dict[str, "automl.containers.models.ModelContainer"],
    deep: bool = True,
) -> str:
...
def calculate_unsupervised_metrics(
    metrics: Dict[str, "automl.containers.metrics.MetricContainer"],
    X,
    labels,
